...
\\
  Victor Picheny\\
  INRA, UR875 Math\'ematiques et Informatique Appliqu\'ees Toulouse\\
  F-31326 Castanet-Tolosan, France\\
  E-mail: \email{victor.picheny@toulouse.inra.fr}\\
}
...
Jointly with modeling efforts, methods have been developed for the exploration and analysis of those simulators, 
...
They may be integrated in various strategies, see e.g., \citet{Wang2007}, \citet{Santana-Quintero2010}, \citet{Tabatabaei2015} and references therein.
...
The \pkg{GPareto} package proposes Gaussian-Process based sequential strategies to solve multi-objective optimization (MOO) problems
...
$y^{(1)}(\x), \ldots, y^{(q)}(\x)$ (where $y^{(i)}: \Xset \rightarrow \mathbb{R}$), that are optimized simultaneously.
...
However, most works are dedicated to mono-objective optimization with large budgets. For small budgets, the packages \pkg{DiceOptim} \citep{Ginsbourger2015} and \pkg{tgp} \citep{Gramacy2007,Gramacy2010}
propose GP-based techniques, but for mono-objective problems only.


On the other hand, GP-based MOO has generated recently a substantial activity in the statistical and optimization communities, with focuses either on sampling strategies \citep{Ponweiser2008a,Wagner2010,Svenson2011,Emmerich2011,Picheny2013,Zuluaga2013} or on uncertainty quantification \citep{Bhardwaj2012,Calandra2014,Binois2015}.
...
\pkg{GPareto} is built upon the \pkg{DiceKriging} \citep{Roustant2012} package that offers Kriging model training for computer experiments.
...
\pkg{GPareto} shares many aspects with those packages.
...
with (unknown) mean $\mu(\x)$ and covariance $c(\x, \x')$.
...
\begin{itemize}
 ...
 \item[Step 2] Fit the GP model to the data, by estimating the mean $\mu(\x)$ and covariance $c(\x, \x')$. Typically, a parametric form is assumed for those functions, whose parameters are adjusted using maximum likelihood approaches.
 The GP model is the distribution of $Y(\x)$ conditional on the observations $y_1, \ldots, y_n$, assuming that $\mu(\x)$ and covariance $c(\x, \x')$ are estimated accurately.
 ...
 \item[Step 4] A new observation $y\+=y(\x\+)$ is obtained by running the simulator and the GP model is updated by conditioning on $f\+$. At this step, the estimates of $\mu$ and $c$ might be updated.
\end{itemize}
Steps 3 and 4 are repeated until the simulation budget is exhausted or when a stopping criterion is met.

\pkg{GPareto} focuses on Steps 3 and 4. Hence, its main entry function, \code{GParetoptim}, requires a GP model as an input. 
Note, however, that \pkg{GPareto} also provides a ``black-box'' entry function, namely \code{easyGParetoptim}, that encompasses Steps 1 to 4
(see the dedicated Section \ref{sec:easyGParetoptim}).
...
For Step 2, \pkg{GPareto} relies on the \pkg{DiceKriging} package, which offers a large variety of mean and covariance functions. 
...
The improvement is defined as the difference between the minimum of the observations and the new function value, such that for a GP model, EI is the conditional expectation of the improvement provided by a new observation $Y(\x)$: 
\begin{equation*}
 EI(\x) = \mathbb{E}\left[ \max(0, \min\limits_{1 \leq i \leq n} (y_i - Y(\x)) \Big| Y(\x_1) = y_1, \dots, Y(\x_n) = y_n \right],
\end{equation*}
which has a closed form expression \citep[see][for calculations]{Jones1998}.

\subsubsection{The Multi-objective case}
When multiple objectives are considered ($f$ has values in $\Rset^q$), Steps 2 and 3 need to be modified. 
...
\pkg{GPareto} focuses on approaches where GP models are fitted independently to each objective. 
Although it is possible to account for correlation between the different objectives (for instance using 
co-kriging models \citep[e.g.,][]{Alvarez2011,Kleijnen2014}, \citet{Svenson2011} showed that it provided little benefit compared to the 
additional complexity. 

Choosing infill points from a set of GP models is a complex question (see Section \ref{sec:MOO}). Within \pkg{GPareto}, 
...
In a similar fashion, a multi-objective improvement function can be defined by estimating the expected ``progress'' brought by a new observation (relatively to the current set of non-dominated observations $\mathcal{P}_n$).
...
These indicators, among others, have been used to define ``generalized'' expected improvements.
...
Finally, the Stepwise Uncertainty Reduction (SUR) criterion of \cite{Picheny2013} is in turn concerned with the probability of non-domination (also called probability of improvement),
...
In short, it exploits the capacity of the GP models to generate different possible paths $S_1^{(1)}, \ldots, S_1^{(q)}, \ldots, S_N^{(1)}, \ldots, S_N^{(q)}$ for the outputs (that is, realizations of GPs conditional on the observations, see Figure \ref{fig:GPsim}).
For each path, a Pareto front is obtained (say, $\mathcal{P}_1, \ldots, \mathcal{P}_N$). Then, the set of fronts are used to define an average set $\mathcal{\bar P}$ estimating the true Pareto front while the deviation from this set is used as a measure of uncertainty. 
...
\caption{Left and center: three conditional simulations of objectives $f_1$ and $f_2$, respectively. Right: corresponding images in the objective space. 
Pareto sets and fronts are shown in bold.}%
\label{fig:GPsim}%
\end{figure}

As described in \citet{Binois2015}, this approach requires conditional simulations on discrete sets of inputs (for instance, a grid or a space-filling sample, 
...
As for the Pareto front, returning a smooth estimate of the entire Pareto set $\mathbb{X}^*$ may be useful to practitioners. We propose here to rely on two complementary approaches. 

First, as for the Pareto front, conditional simulations can be used here: from each set of GP realizations, the Pareto set $\mathbb{X}^*_i$ can be obtained.
...
\subsubsection{User-friendly wrapper: easyGParetoptim}
...
A design of experiments may be passed using the argument \code{par} \textemdash otherwise it is a Maximin LHS design constructed from \pkg{DiceDesign} \textemdash and corresponding values provided with \code{values}. Additional tuning of the inner procedures are available using the \code{control} list whose elements are detailed next.  

\subsubsection{GParetoptim}
...
More setting parameters are directly exposed, such as \code{crit} for selecting the infill criteria or \code{cov.reestim} to decide whether or not hyperparameters are updated after adding new observations. 
As for \code{easyGParetoptim}, more flexibility is given using control parameters, \code{optim\_control} for the optimization of the infill criterion and \code{crit\_control} for parameters of this latter, 
that are useful for the following \code{crit\_optimizer} function. 

\subsubsection{crit optimizer}
...
Three solutions to perform this inner optimization are provided in \pkg{GPareto}: 
\begin{enumerate}
...
\subsubsection{crit functions}
\label{subsub:crit}
Four criteria are available in \pkg{GPareto} 1.0.2: 
\begin{itemize}
	\item \code{crit_SMS} for the SMS-EGO criterion \citep{Ponweiser2008a,Wagner2010} (based on the Matlab source code of the authors);
	\item \code{crit_EHI} for the Expected Hypervolume Improvement criterion \citep{Emmerich2011} (based on the Matlab source code of the authors for the bi-objective case);
...
\end{itemize}
The \code{crit_SMS} criterion has an analytical expression for any number of objectives while the one for \code{crit_EHI} is only for the bi-objective case. There is a semi-analytical\footnote{numerical quadrature is needed for some 1-dimensional integrals, see \cite{Svenson2011}.} formula for \code{crit_EMI} but it has not been implemented in \pkg{GPareto} yet. Note that the formula for \code{crit_EHI} is coded using \pkg{Rcpp}  \citep{Eddelbuettel2011,Eddelbuettel2013}, 
which offers considerable speed-up over an \proglang{R} implementation.\\ 
 
For \code{crit_EMI} and \code{crit_EHI} with $m > 2$, computations rely on Sample Average Approximation (SAA) \citep{Shapiro2003}, as proposed e.g., in \cite{Svenson2011}. 
...
For now \code{crit_SUR} is available for two or three objectives.\\
...
\code{crit\_EMI} & Additive-$\epsilon$  & No             & Any                  & ++ & Yes & \code{nb.samp}\\ \hline
\code{crit\_SMS} & Hypervolume                      & Yes            & Any                  & +  & Yes & \code{refPoint} \\ \hline
\multirow{ 2}{*}{\code{crit\_SUR}} & Pr. of non         & \multirow{ 2}{*}{No}   & \multirow{ 2}{*}{$m \leq 3$}  &   \multirow{ 2}{*}{+++}  & \multirow{ 2}{*}{No} & \code{integration}\\
& domination & & & & & \code{points}
\end{tabular}
\end{table}

Test functions are provided in \pkg{GPareto}, such as problems in the MOP \citep{VanVeldhuizen1999},  ZDT \citep{Zitzler2000} and DTLZ \citep{Deb2005} test suites.  

\subsection{Functions related to uncertainty quantification and post-processing}\label{sec:pkgUQ}

\subsubsection{User-friendly wrapper: plotGPareto}
...
Motivated by applications where some objective function are computable at a negligible cost compared to other objectives, \pkg{GPareto} offers an option for MOO in case of co-existing cheap- and expensive-to-evaluate objectives. 
...
\begin{itemize}
	...
	\item minimal ``canonical distance'' associated with $k_n$: $\min \limits_{1 \leq i \leq n} \sqrt{k_n(\x, \x) - 2 k_n(\x, \x_i) + k_n(\x_i, \x_i)}$.
\end{itemize} 
...
Moreover, to improve stability of the update of already existing models with new observations, it is possibly attempted twice. First, an update with re-estimation of the hyperparameters is performed. Then, if it has failed, a new update is tested with the old hyperparameters. If this is still insufficient to train the model with all observations, the user may try to remove some points or apply the \emph{jitter} technique consisting in adding a small constant to the diagonal of the covariance matrix to improve its condition number, see e.g., \cite{Roustant2012}. Replacing two close observations by one observation and its estimated derivative as proposed in \cite{Osborne2010} is another appealing solution.\\ 
...
We first define the initial design of experiments (\texttt{design.init}, six points evenly spaced between zero and one) and compute the corresponding set of observations \texttt{response.init}, which we use to build two kriging models with \pkg{DiceKriging}'s \texttt{km} function and put them into a single list (\texttt{model}):
...
@
Then, we call the main function \texttt{GParetoptim} to perform seven optimization steps using the EHI criterion. 
Note that EHI requires a \textit{reference point} as a parameter, which corresponds to an 
upper bound for each objective (here $[2,2]$, if not provided, it is estimated at each iteration, see Section \ref{subsub:crit}). The other mandatory inputs are the GP models \texttt{model}, the objective function \texttt{fn} number of steps (\texttt{nsteps}) and the design bounds (\texttt{lower} and \texttt{upper}).
<<warning=FALSE>>=
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Two objectives, two dimensions example}
We consider now the more complex optimization problem (P1) given in \cite{Parr2012}:
$$\text{P1}(x)=\left\{ \begin{array}{l l}
 f_1=(x_2-5.1(x_1/(2\pi))^2+\frac{5}{\pi}x_1-6)^2 +10((1-\frac{1}{8\pi})\cos(x_1)+1)\\
 f_2=-\sqrt{(10.5-x_1)(x_1+5.5)(x_2+0.5)} - \frac{(x_2 -5.1(x_1/(2\pi))^2-6)^2}{30} - \frac{(1-1/(8\pi))\cos(x_1)+1}{3}
\end{array} \right. $$
...
\begin{itemize}
  ...
  \item the use of \textit{fastfun} objects and
  \item the post-processing functionalities.
\end{itemize}

On this analytical example, it is possible to display the true Pareto front and set using the \texttt{plotParetoGrid} function:
...
\begin{figure}[htpb]
\centering
	\includegraphics[trim=10mm 10mm 10mm 1mm, clip, width=\textwidth]{figure/P1-1.pdf}%
...
Now, we call directly the function \texttt{crit\_optimizer} to choose the next point to evaluate using the SUR criterion. 
Here, the \texttt{optimcontrol} input is used to choose the \texttt{genoud} algorithm
for the criterion optimization. The \texttt{critcontrol} input allows us to choose the integration points for the criterion,
...
We split the objective into two separate functions, \texttt{fun1} and \texttt{fun2},
and we replace the second GP model by a \texttt{fastfun} object:
<<echo=TRUE,message=FALSE,warning=FALSE, results='hide'>>=
...
%to illustrate the influence of the choice of criterion and of inner optimization method. 
In Figure \ref{fig:ex2}, we show  the initial set of observations and the next point to evaluate according to each setup.
For illustration purposes, the contour lines of the criteria are also computed. We see that using the \texttt{fastfun}
...
Then, we generate the post-treatment processes using \texttt{plotGPareto}. 
The graphical outputs are given in Figure \ref{fig:ex3}. 
Optional parameters \texttt{f1lim} and \texttt{f1lim} are used to fix bounds for the top graphs to allow better comparison.
...
plotGPareto(solFast, UQ_PF = TRUE, UQ_PS = TRUE, UQ_dens = TRUE,
            control = list(f1lim = lim1, f2lim = lim2))
@
First, we see the interest of using the \texttt{fastfun} class when some objectives are cheap to compute:
...
Overall the Vorob'ev deviation values (429 and 145, resp.\@) indicate a substantially better confidence on the predicted Pareto front
using \texttt{fastfun}.
...
Here, we have chosen a target $[33.5,-26.5]$ that is on the Vorob'ev expectation, where the uncertainty is small but where no observation is near (Figure \ref{fig:ex3}, top left). The \texttt{getDesign} output is a list with the value of the design (\texttt{par}), the value of the criterion, i.e., the probability that the \texttt{newPoint} objective is not dominated by the target) (\texttt{value}, here 50\%) and the GP prediction of each objective with the associated uncertainty ((\texttt{mean}), (\texttt{sd}) and confidence intervals). Here, the value of the second objective reaches the target with large confidence, but the first objective value is quite uncertain.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
This time we simply use \texttt{easyGParetoptim} to solve the problem without having to train or prepare models.
...
\centering
	\includegraphics[trim=10mm 10mm 10mm 10mm, clip, width=\columnwidth]{figure/ex3DPS-1.pdf}%
\end{subfigure}
\begin{subfigure}[t]{0.6\linewidth}
\centering
  \includegraphics[trim=5mm 10mm 10mm 10mm, clip,   width=\columnwidth]{figure/ex3DPS-2.pdf}%
\end{subfigure}
