...
\label{subsub:crit}
Four criteria are available in \pkg{GPareto} 1.0.3: 
\begin{itemize}
...
Optional parameters \texttt{f1lim} and \texttt{f2lim} are used to fix bounds for the top graphs to allow better comparison.
...
Overall the Vorob'ev deviation values (368 and 296, resp.\@) indicate a substantially better confidence on the predicted Pareto front
using \texttt{fastfun}.
