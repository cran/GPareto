...
By default, \code{easyGParetoptim} uses \code{SMS} as criterion, with \code{pso} as inner-optimization routine. This choice has been made to favor speed. They are also shared by the following \code{GParetoptim} routine.
...
The \code{crit_SMS} criterion has an analytical expression for any number of objectives while the one for \code{crit_EHI} is only for the bi-objective case. There is a semi-analytical\footnote{numerical quadrature is needed for some 1-dimensional integrals, see \cite{Svenson2011}.} formula for \code{crit_EMI}. Note that the formula for \code{crit_EHI} is coded using \pkg{Rcpp}  \citep{Eddelbuettel2011,Eddelbuettel2013}, 
...
Overall the Vorob'ev deviation values (356 and 293, resp.\@) indicate a substantially better confidence on the predicted Pareto front
using \texttt{fastfun}.
